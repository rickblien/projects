...
def main():
    ...
    with features:
        ...
        start = st.text_input("Start Date", '1950-01-01') # create start date input box, set '2020-02-11' = default
        ...
        if not ticker:
            ...
            with st.spinner('Loading data......'):
                ...
                st.subheader("The CDF of the PE for {} is {}".format(ticker,PE_CDF))
...
def setup():
    ...
    Fundamental_URL = ("https://ycharts.com/companies/")
...
def grab_historical_EPS(ticker):
    ...
    table1 = pd.read_html(Fundamental_URL+ticker+"/eps",attrs={"class":"histDataTable"})[0]
    earnings1 = pd.DataFrame(data = table1, columns=None)
    earnings1.columns = ['Date','EPS']

    table2 = pd.read_html(Fundamental_URL+ticker+"/eps",attrs={"class":"histDataTable"})[1]
    earnings2 = pd.DataFrame(data = table2)
    earnings2.columns = ['Date','EPS']

    all_earnings = pd.concat([earnings1, earnings2], ignore_index=True) #combine earnings tables
    all_earnings['Date'] = pd.to_datetime(all_earnings['Date'])  #make date column in to datetime object
...
def suggestion(CDF,measure):
    ...
    if measure == "PE":
        if CDF > .5:
            st.markdown('We suggest to **SELL**.')
        elif CDF == .5:
            st.markdown('We suggest to **HOLD**.')
        else:
            st.markdown('We suggest to **BUY**.')

    elif measure == "Volatility":
        if CDF > .5:
            st.markdown('**Sell Call** or **Buy Put**')
        elif CDF == .5:
            st.markdown('**Buy** or **Sell** **Call** and **Put**')
        else:
            st.markdown('**Buy Call** or **Sell Put**')
